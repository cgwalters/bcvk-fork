...
static TEST_CLOUD_INIT_CONFIGDRIVE_BASIC: IntegrationTest = IntegrationTest::new(
    "test_cloud_init_configdrive_basic",
    test_cloud_init_configdrive_basic,
);
...
"#;
    ...
    let output = run_bcvk(&[
        ...
        &cloud_config_path,
        "--execute",
        "lsblk -o NAME,SERIAL,SIZE && ls -la /dev/disk/by-id/virtio-config-2 && mkdir -p /mnt/configdrive && mount /dev/disk/by-id/virtio-config-2 /mnt/configdrive && ls -la /mnt/configdrive/ && cat /mnt/configdrive/openstack/latest/user_data && cat /mnt/configdrive/openstack/latest/meta_data.json",
        &test_image,
    ])?;
    ...
    // Verify ConfigDrive device exists
    assert!(
        combined_output.contains("config-2"),
        "ConfigDrive device with serial 'config-2' not found in lsblk output. Output: {}",
        combined_output
    );
    ...
    // Verify meta_data.json contains instance-id
    assert!(
        combined_output.contains("instance-id"),
        "meta_data.json does not contain instance-id. Output: {}",
        combined_output
    );
...
#[distributed_slice(INTEGRATION_TESTS)]
static TEST_CLOUD_INIT_NO_DATASOURCE_CMDLINE: IntegrationTest = IntegrationTest::new(
...
#[distributed_slice(INTEGRATION_TESTS)]
static TEST_CLOUD_INIT_CONFIGDRIVE_CONTENT: IntegrationTest = IntegrationTest::new(
    "test_cloud_init_configdrive_content",
    test_cloud_init_configdrive_content,
);
...
"#;
    ...
    let output = run_bcvk(&[
        ...
        &cloud_config_path,
        "--execute",
        "mkdir -p /mnt && mount /dev/disk/by-id/virtio-config-2 /mnt && cat /mnt/openstack/latest/user_data",
        &test_image,
    ])?;
