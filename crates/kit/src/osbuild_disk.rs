...
impl OsbuildDiskOpts {
    ...
    fn generate_bib_command(&self, config_in_vm: Option<&str>) -> Result<Vec<String>> {
        ...
        let script = indoc! {r#"
            ...
            mount -t tmpfs -o size=20G tmpfs /var/lib/containers

            # Configure VM's podman to use host storage as additional image store
            # This allows podman to find both the b-i-b image and source image
            # Match the to-disk pattern which just uses STORAGE_OPTS without runroot mount
            ...
            # Create storage.conf for b-i-b to use the additional image store
            # B-I-B uses containers/storage Go library which reads /etc/containers/storage.conf
            cat > /tmp/storage.conf <<'STORAGECONF'
[storage]
driver = "overlay"
...
STORAGECONF
            ...
            # Execute b-i-b with VM's local container storage
            # Mount storage.conf so b-i-b can find images in the additional image store
            # Mount at /var/lib/containers/storage specifically (not the parent dir)
            # Use /var/tmp for output (writable local tmpfs), then copy to virtiofs mount
            echo "Running bootc-image-builder..."
            mkdir -p /var/tmp/bib-output
            podman run --rm -i ${tty} --privileged \
                --security-opt label=type:unconfined_t \
                -v /var/lib/containers/storage:/var/lib/containers/storage \
                -v /var/tmp/bib-output:/output \
                -v ${AIS}:${AIS} \
            ...
            cp -r /var/tmp/bib-output/* /run/virtiofs-mnt-output/
